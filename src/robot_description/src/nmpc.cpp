...
double NMPC::CostFunction(TRobotStateSim Robot, TTargetStateSim Target, Matrix2d& Ut){
    ...
    for(int i=N1; i<=N2; i++){
        ...
        sum_cost = sum_cost + L1*fabs(DVAL - RobotTargetDist);

        sum_cost = sum_cost + L2*fabs(DiffAngle(RobotTargetAngle,Robot.teta));
    }

    deltaU = L3*(fabs(Robot.v-Ut(0,0))+fabs(Robot.w-Ut(1,0)));
    res = (sum_cost + deltaU);
